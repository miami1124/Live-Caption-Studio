...
**⚠️ 請主動告知使用者這件事**：使用免費 Gemini 額度時，Google 條款明訂輸入輸出可能被用於改善產品、也可能由人工審閱，並要求不要送機密或個人敏感資訊（[官方條款](https://ai.google.dev/gemini-api/terms)）。這個工具傳送的是使用者現場講話的內容，所以如果他要報告的是公司內部資料或未公開研究，請建議他改用付費專案。
